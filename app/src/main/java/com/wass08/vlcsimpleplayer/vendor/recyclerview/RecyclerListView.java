...
/**
 * Wraps {@link RecyclerListView} providing interface similar to `ScrollView.js` where each children
 * will be rendered as a separate {@link RecyclerListView} row.
 * <p>
...
public class RecyclerListView extends RecyclerView {
    ...
    public void scrollToPositionDelayed(final int position) {
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                RecyclerListView.this.scrollToPosition(position);
            }
        }, 300);
    }
     ...
     */
    private static class ConcreteViewHolder extends android.support.v7.widget.RecyclerView.ViewHolder {
    ...
    /**
     * View that is going to be used as a cell in {@link RecyclerListView}. It's going to be reusable and
    ...
    /**
     * JavaScript MyListView implementation rely on getting correct scroll offset. This class helps
    ...
    private static class ScrollOffsetTracker {
        ...
        public int getTopOffsetForItem(int index) {
            // This method is frequently called from the "onScroll" handler of the "RecyclerListView" with an
    ...
    /*package*/ static class ReactListAdapter extends Adapter<ConcreteViewHolder> {
        ...
        private final RecyclerListView mScrollView;
        ...
        private final View.OnLayoutChangeListener
            ...
            public void onLayoutChange(
                ...
                if (oldHeight != newHeight) {
                    ...
                    // To achieve that we call `forceLayout()` on the view modified and on `RecyclerListView`
                    // instance (which is accessible with `v.getParent().getParent()` if the view is
                    // attached). We rely on NativeViewHierarchyManager to call `layout` on `RecyclerListView`
                    // then, which will happen once all the children of `RecyclerListView` have their layout
        ...
        public ReactListAdapter(RecyclerListView scrollView) {
